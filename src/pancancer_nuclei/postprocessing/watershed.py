...
def boundary_predictions_to_instances(
    ...
    foreground = (
        (semantic_mask > 0)
        & (semantic_mask < NUMBER_OF_SEMANTIC_CLASSES)
        & (semantic_mask != IGNORE_LABEL)
    )
    ...
    watershed_map = ndimage.watershed_ift(elevation, markers, structure=structure)
    watershed_map[~foreground] = 0
    ...
    for marker_id in np.unique(watershed_map):
        ...
        semantic_labels = semantic_mask[instance_mask]
        semantic_labels = semantic_labels[
            (semantic_labels > 0) & (semantic_labels < NUMBER_OF_SEMANTIC_CLASSES)
        ]
        ...
        semantic_label = int(
            np.bincount(
                semantic_labels.astype(np.int64),
